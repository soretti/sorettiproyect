<form action="" method="post" id="myform">
  ...
  <div class="row">
      <div class="col-md-8">
        ...
        <a class="btn btn-primary"  href="<?php echo base_url('modulo/blog/agregar/'.$pagina_id) ?>"><i class="glyphicon glyphicon-plus icon-white"></i> Agregar</a>
        <?php } ?>
        <?php if($this->acceso->valida('pagina','eliminar')) {?>
        <a class="btn btn-danger seleccionados" href="<?php echo base_url('modulo/blog/eliminar/'.$pagina_id) ?>" ><i class="glyphicon glyphicon-trash icon-white"></i> Eliminar seleccionados</a>
        <?php } ?>
      </div>
          ...
          <table class="table table-bordered grid">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td nowrap>
                  <button class="btn  btn-default btn-sm seleccionar" type="button" value="<?php echo $pagina->id; ?>" titulo="<?php echo $pagina->titulo; ?>" uri="<?php echo base_url('blog/'.$bdpagina->uri.'/'.$pagina->uri.".html"); ?>" >Insertar</button>
                  <?php if($this->acceso->valida('pagina','consultar')) {?>
                  <a href="<?php echo base_url('modulo/blog/editar/'.$pagina_id."/".$pagina->id); ?>" class="btn btn-default btn-sm"><i class="glyphicon glyphicon-edit"></i></a>
                  <?php } ?>
                  <?php if($this->acceso->valida('pagina','eliminar')) {?>
                  <button class="btn btn-danger btn-sm  action_row" value="basura" type="button" href="<?php echo base_url('modulo/blog/eliminar/'.$pagina_id); ?> " ><i class="glyphicon glyphicon-trash"></i></button>
                  <?php } ?>
                </td>
              </tr>
              <?php } ?>
