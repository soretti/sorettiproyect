            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="btn-toolbar">
                        ...
                        <button class="btn btn-success" type="submit" onclick="goto('<?php echo base_url('modulo/'.$this->uri->segment(2).'/guardar/'.$bloque->id.'/'.$id_page) ?>')">Guardar</button>
                        <?php } ?>
                        ...
                        <?php if( $tipoBloque->configuracion['max-items'] > $bloque->bloquecontenidos->where('is_enable',1)->count() ||  $tipoBloque->configuracion['max-items']==0   ) {
                                if($bloque->id!=6){?>
                                <a class="btn btn-primary" href="<?php echo base_url('modulo/'.$this->uri->segment(2).'/agregar_bloque/'.$bloque->id.'/'.$id_page) ?>">Agregar</a>
                        <?php } } ?>
                         ...
                         <?php foreach ($bloques as $value) {?>
                                <div class="form-group">
                                     <div class="input-group col-md-6">
                                      ...
                                      <span class="input-group-addon"><a href="<?php echo base_url('modulo/'.$this->uri->segment(2).'/editar_bloque/'.$bloque->id.'/'.$value->id.'/'.$value->pagina_id) ?>" class="filemanager"> <i class="glyphicon glyphicon-edit "></i></a></span>

                                    </div>

                                      <?php if($value->visible==0){ ?>
                                        <div class="input-group visible-green col-md-6">
                                        Visible&nbsp;<i class="glyphicon glyphicon-eye-open" aria-hidden="true"></i>
                                        </div>
                                      <?php }else{ ?>
                                        <div class="input-group novisible-red col-md-6">
                                        No visible&nbsp;<i class="glyphicon glyphicon-eye-close" aria-hidden="true"></i>
                                        </div>
                                      <?php } ?>
