...
class Sliderpage_controller extends Bloque_controller
{
  	public function __construct()
    {
        ...
        $this->load->model('sliderpage','ContenidoBloque');
	...
	public function index()
	{
                          ...
                          $bloque = $this->_data(20);


                          //print_r($bloque); die();


		$sliders = $bloque->bloquecontenidos->is_active()->where("pagina_id",$pagina_id)->where("visible", 0)->order_by('sort','ASC')->get();

        //print_r($sliders); die();

        // foreach ($sliders as $key => $value) {
        //     echo 'fff:'.$value->imagen;
        // }
        // die();
		$this->load->view('sliderpage',array('sliders'=>$sliders , 'bloques'=> $bloque, 'pagina_id'=>$pagina_id ));
	}

     public function editar_bloque($id_bloque,$id=0){

        $id_page=$this->uri->segment(6);
        //echo $id_page; die();
     ...
     public function guardar_bloque($id_bloque)
    {
        ...
        $rel = $bloque->from_array($_POST, array('titulo','visible','visible_titulo','subtitulo','texto','imagen','imagen2','liga','target','bloque_id','fecha_creacion','fecha_activacion','fecha_desactivacion','titulo_en','texto_en','pagina_id'));
        if(!$id) $bloque->sort='0';
